...
#define NAL_HEADER_DERIVE_METHOD(fn)                                                               \
    bool SmolRTSP_NalHeader_##fn(SmolRTSP_NalHeader h) {                                           \
        bool result = 0;                                                                           \
                                                                                                   \
        match(h) {                                                                                 \
            of(SmolRTSP_NalHeader_H264, h264_h) result = SmolRTSP_H264NalHeader_##fn(*h264_h);     \
            of(SmolRTSP_NalHeader_H265, h265_h) result = SmolRTSP_H265NalHeader_##fn(*h265_h);     \
        }                                                                                          \
                                                                                                   \
        return result;                                                                             \
    }
...
SmolRTSP_NalHeader SmolRTSP_NalUnit_header(SmolRTSP_NalUnit nal_unit) {
...
U8Slice99 SmolRTSP_NalUnit_payload(SmolRTSP_NalUnit nal_unit) {
    U8Slice99 result = {0};
