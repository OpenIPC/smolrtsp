 ...
 * equal.
 */
#define ASSERT_EQ(actual, expected) NALA_ASSERT_FUNC(actual)((actual), (expected), NALA_CHECK_EQ)

/**
 * Assert that given characters, numbers, pointers or strings are not
 * equal.
 */
#define ASSERT_NE(actual, expected) NALA_ASSERT_FUNC(actual)((actual), (expected), NALA_CHECK_NE)

/**
 * Assert that actual is less than expected.
 */
#define ASSERT_LT(actual, expected) NALA_ASSERT_FUNC(actual)((actual), (expected), NALA_CHECK_LT)

/**
 * Assert that actual is less than or equal to expected.
 */
#define ASSERT_LE(actual, expected) NALA_ASSERT_FUNC(actual)((actual), (expected), NALA_CHECK_LE)

/**
 * Assert that actual is greater than expected.
 */
#define ASSERT_GT(actual, expected) NALA_ASSERT_FUNC(actual)((actual), (expected), NALA_CHECK_GT)

/**
 * Assert that actual is greater than or equal to expected.
 */
#define ASSERT_GE(actual, expected) NALA_ASSERT_FUNC(actual)((actual), (expected), NALA_CHECK_GE)
 ...
 */
#define ASSERT_SUBSTRING(haystack, needle) nala_assert_substring(haystack, needle)

/**
 * Assert that given haystack string does not contain given needle
 * string.
 */
#define ASSERT_NOT_SUBSTRING(haystack, needle) nala_assert_not_substring(haystack, needle)

/**
 * Assert that given memory regions are equal.
 */
#define ASSERT_MEMORY_EQ(actual, expected, size) nala_assert_memory(actual, expected, size)

/**
 * Assert that given files are equal.
 */
#define ASSERT_FILE_EQ(actual, expected) nala_assert_file_eq(actual, expected)
...
#define ASSERT_FUNCTION_POINTER_EQ(actual, expected)                                               \
    if (actual != expected) {                                                                      \
        nala_test_failure(nala_format("%p is not equal to %p", actual, expected));                 \
    }
...
#define ASSERT_FUNCTION_POINTER_NE(actual, expected)                                               \
    if (actual == expected) {                                                                      \
        nala_test_failure(nala_format("%p is equal to %p", actual, expected));                     \
    }
...
#define CAPTURE_OUTPUT(stdout_name, stderr_name)                                                   \
    ...
    static char *stderr_name = NULL;                                                               \
                                                                                                   \
    for (NALA_UNIQUE(stdout_name) = 0, nala_capture_output_start(&stdout_name, &stderr_name);      \
         NALA_UNIQUE(stdout_name) < 1; NALA_UNIQUE(stdout_name)++, nala_capture_output_stop())
...
#define WITH_MESSAGE(format, ...)                                                                  \
    int NALA_UNIQUE(i);                                                                            \
    for (NALA_UNIQUE(i) = 0, nala_with_message_push(format, ##__VA_ARGS__); NALA_UNIQUE(i) < 1;    \
...
#define TEST(name)                                                                                 \
    ...
    static struct nala_test_t nala_test_##name = {.name_p = #name,                                 \
                                                  ...
                                                  .next_p = NULL};                                 \
    __attribute__((constructor)) static void nala_constructor_##name(void) {                       \
...
const char *
nala_format_memory(const char *prefix_p, const void *actual_p, const void *expected_p, size_t size);
...
void nala_assert_array(const void *actual_p, const void *expected_p, size_t item_size, size_t size);
